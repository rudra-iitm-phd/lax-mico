...
def sac_train_step(
    ...
    next_act, next_log_prob = actor(critic.encoder_state(next_obs), key)
    ...
    def critic_loss_fn(critic: Critic):
        ...
        # loss = jnp.mean(optax.huber_loss(q1, backup, delta=1.0)) + jnp.mean(
        #     optax.huber_loss(q2, backup, delta=1.0)
        # )
    ...
    g_sx_next, g_xs_next = target_state_metric(s, x)
    ...
    def rep_loss_fn(critic: Critic):
        phi_s, phi_x = critic.encoder_state(s), critic.encoder_state(x)
        state_phi_diff = optax.huber_loss(phi_s, phi_x, delta=1.0)
        g_sx, g_xs = target_state_metric(s, x)
        lax_mico_state_diff = jnp.maximum(g_sx, g_xs)

        loss1 = jnp.mean(
            (state_phi_diff - jax.lax.stop_gradient(lax_mico_state_diff)) ** 2
        )

        phi_sa, phi_xb = (
            critic.encoder_state_action(s, a),
            critic.encoder_state_action(x, b),
        )
        state_action_phi_diff = optax.huber_loss(phi_sa, phi_xb, delta=1.0)
        lax_mico_state_action_diff = lambda_target

        loss2 = jnp.mean(
            (state_action_phi_diff - jax.lax.stop_gradient(lax_mico_state_action_diff))
            ** 2
        )

        return loss1 + loss2

    def critic_and_rep_loss_fn(critic: Critic):
        critic_loss, (q1_mean, q2_mean) = critic_loss_fn(critic)
        rep_loss = rep_loss_fn(critic)
        total_loss = critic_loss + rep_loss
        aux = {
    ...
    (critic_and_rep_loss, aux), critic_and_rep_grads = nnx.value_and_grad(
        critic_and_rep_loss_fn, has_aux=True
    )(critic)
    critic_opt.update(critic, critic_and_rep_grads)

    critic_loss, rep_loss, q1_mean, q2_mean = (
        aux["critic_loss"],
        aux["rep_loss"],
        aux["q1_mean"],
        aux["q2_mean"],
    ...
    # actor_grads.state_encoder = jax.tree.map(jnp.zeros_like, actor_grads.state_encoder)
    ...
    def value_diff(
        ...
        anchor_act: jnp.ndarray,
        obs_batch: jnp.ndarray,
        act_batch: jnp.ndarray,
    ):
        anchor_obs, anchor_act = anchor_obs[None, :], anchor_act[None, :]

        qa1, qa2 = critic(anchor_obs, anchor_act)
        q_anchor = jnp.minimum(qa1, qa2)

        qb1, qb2 = critic(obs_batch, act_batch)
        q_batch = jnp.minimum(qb1, qb2)
        return jnp.mean(jnp.abs(q_anchor - q_batch))

    def state_action_diff(
        anchor_obs: jnp.ndarray,
        anchor_act: jnp.ndarray,
        obs_batch: jnp.ndarray,
        act_batch: jnp.ndarray,
        ...
        anchor_obs, anchor_act = anchor_obs[None, :], anchor_act[None, :]
        anchor_sa = jnp.concatenate([anchor_obs, anchor_act], axis=-1)
        anchor_sa = jnp.repeat(anchor_sa, obs_batch.shape[0], axis=0)
        dab, dba = state_action_metric(
            anchor_sa, jnp.concatenate([obs_batch, act_batch], axis=-1)
        )
        return jnp.mean(jnp.maximum(dab, dba))

    val_diff = value_diff(s[0], a[0], s, a)
    state_diff = state_action_diff(s[0], a[0], s, a)
...
def main(args, cfg_env=None):
    ...
    while steps < config["total_env_steps"]:
        ...
        logger.log_tabular("Analysis/Val_diff", val_diff.item())
        logger.log_tabular("Analysis/SA_diff", state_diff.item())
    ...
    logger.nn_model_save(
        itr=steps, nn_model_saver_element=state_action_metric, prefix="sa_metric"
    )
    logger.nn_model_save(
        itr=steps, nn_model_saver_element=state_metric, prefix="s_metric"
    )
    logger.close()
