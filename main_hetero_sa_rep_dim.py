...
from utils.acting import actor_step_rep, wrap_env_for_training
...
def sac_train_step(
    ...
    def state_metric_loss_fn(state_metric: EnsembleStateMetric):
        ...
        target_sa_rep, target_xb_rep = (
            target_rep.state_action_rep(s, a),
            target_rep.state_action_rep(x, b),
        )
        target_s_rep, target_x_rep = target_rep.state_rep(s), target_rep.state_rep(x)
        h_sax, h_xbs = (
            target_state_action_to_state_metric(target_sa_rep, target_x_rep),
            target_state_action_to_state_metric(target_xb_rep, target_s_rep),
        )
    ...
    def rep_loss_fn(rep_net: RepNet):
        ...
        # learn the representation space such that L_1(phi(s)-phi(x)) = U(phi(s), phi(x))
        s_rep, x_rep = rep_net.state_rep(s), rep_net.state_rep(x)

        target_s_rep, target_x_rep = target_rep.state_rep(s), target_rep.state_rep(x)
        g_sx, g_xs = target_state_metric(target_s_rep, target_x_rep)
        ...
        # also learn the representation space such that L_1(phi(s,a)-phi(x, b)) =  lambda((s, a), (x, b))

        sa_rep, xb_rep = rep_net.state_action_rep(s, a), rep_net.state_action_rep(x, b)

        sa_diff = jnp.linalg.norm(sa_rep - xb_rep, axis=-1, ord=1)

        loss_sa = jnp.mean(
            optax.huber_loss(sa_diff, jax.lax.stop_gradient(lambda_target), delta=1.0)
        )
    ...
    polyak_update(target_rep, rep_net, config.update_tau)
    ...
    def value_diff(
        ...
        a_rep, b_rep = (
            rep_net.state_action_rep(anchor_obs, anchor_act),
            rep_net.state_action_rep(obs_batch, act_batch),
        )
        qa1, qa2 = critic(a_rep)
        q_anchor = jnp.minimum(qa1, qa2)

        qb1, qb2 = critic(b_rep)
    ...
    def state_action_diff(
        ...
        a_rep = jnp.repeat(a_rep, obs_batch.shape[0], axis=0)
        dab, dba = state_action_metric(a_rep, b_rep)
...
def main(args, cfg_env=None):
    ...
    logger.nn_model_save(itr=steps, nn_model_saver_element=rep_net, prefix="rep_net")
    logger.nn_model_save(
