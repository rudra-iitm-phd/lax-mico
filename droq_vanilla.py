...
def sac_train_step(
    ...
    def critic_loss_fn(critic, target_critic):
        next_q_min = target_critic.get_min(jnp.concatenate([obs, act], axis=-1))
        backup = jax.lax.stop_gradient(
            reward + config.gamma * discount * (next_q_min - alpha * next_log_prob)
        )
        q_values = critic(jnp.concatenate([obs, act], axis=-1))
        loss = jnp.mean(jnp.mean((q_values - backup[None, :]) ** 2, axis=-1))
        return (loss, jnp.mean(q_values))

    (critic_loss, q_mean), critic_grads = nnx.value_and_grad(
    ...
    def actor_loss_fn(actor, critic):
        ...
        q_mean = critic.get_mean(jnp.concatenate([obs, pi], axis=-1))
    ...
    return critic_loss, actor_loss, alpha_loss, alpha, log_pi_mean, q_mean
...
def train_n_steps(
    ...
    def body_fun(i, carry):
        ...
        init_val = (jnp.zeros((), jnp.float32),) * 6
        models = (
            actor,
            actor_opt,
            critic,
...
def main(args, cfg_env=None):
    ...
    while steps < config["total_env_steps"]:
        ...
        logger.log_tabular("DroQ/Alpha", alpha.item())
        logger.log_tabular("DroQ/LogPi_mean", log_pi_mean.item())
        logger.log_tabular("DroQ/Q1_mean", q_mean.item())
