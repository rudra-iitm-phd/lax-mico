...
from utils.buffer import (RunningMeanStd, RunningStatistics,
                          UniformSamplingQueue)
from utils.logger import EpochLogger
from utils.models import (EnsembleCritic, SACGaussianActor, Scalar,
                          get_tree_norm)
from utils.parameterized_models import ParameterizedModels
...
def sac_train_step(
    ...
    alpha = jnp.exp(models.get("log_alpha")())

    key, next_key = jax.random.split(key)
    next_act, next_log_prob = models.get("actor")(next_obs, next_key)
    ...
    # )(models.get("critic"), models.get("target_critic"))
    
    # models.get("critic_opt").update(models.get("critic"), critic_grads)

    key, act_key = jax.random.split(key)

    def actor_loss_fn(actor):
        pi, log_pi = actor(obs, act_key)
        q1, q2 = models.get("critic")(jnp.concatenate([obs, pi], axis=-1))
    ...
    models.get("actor_opt").update(models.get("actor"), actor_grads)
    ...
    alpha_loss, alpha_grads = nnx.value_and_grad(alpha_loss_fn)(models.get("log_alpha"))
    models.get("alpha_opt").update(models.get("log_alpha"), alpha_grads)


    polyak_update(models.get("target_critic"), models.get("critic"), config.update_tau)
...
def train_n_steps(
    ...
    def body_fun(i, carry):
        ...
        n_env_state, transition = actor_step(
            env, env_state, models.get("actor"), obs_normalizer, env_key, extra_fields=("truncation",)
        )
        ...
        graphdef, model_state = nnx.split(models)
        def do_train(j, carry):
            key, env_state, buffer_state, obs_normalizer, model_state, _ = carry
            ...
            models_local = nnx.merge(graphdef, model_state)
            val = sac_train_step(models_local, batch, config, train_key)
            model_state = nnx.state(models_local)
            ...
            return (key, env_state, buffer_state, obs_normalizer, model_state, val)

        init_val = (jnp.zeros((), jnp.float32),) * (14 + 4)
        
        key, _, buffer_state, obs_normalizer, model_state, val = nnx.fori_loop(
            0,
            config.train_per_step,
            do_train,
            (key, n_env_state, buffer_state, obs_normalizer, model_state, init_val),
        )

        models = nnx.merge(graphdef, model_state)
...
def main(args, cfg_env=None):
    ...
    models.add("actor", actor)
    models.add("actor_opt", actor_opt)
    models.add("critic", critic)
    models.add("critic_opt", critic_opt)
    models.add("target_critic", target_critic)
    models.add("log_alpha", log_alpha)
    models.add("alpha_opt", alpha_opt)
