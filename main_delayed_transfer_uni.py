...
def sac_train_step(
    ...
    def state_action_metric_loss_fn(state_action_metric: EnsembleStateActionMetric):
        ...
        d_sa_xb_e2, d_xb_e2_sa = state_action_metric(sa_rep, xb_e2_rep)
        lambda_e2_current = jnp.maximum(d_sa_xb_e2, d_xb_e2_sa)
        loss_e2 = jnp.mean((lambda_e2_current - lambda_e2_target) ** 2)

        d_sa_xb_2, d_xb_sa_2 = state_action_metric(sa_e2_rep, xb_e2_rep)
    ...
    def min_state_action_to_state_metric_loss_fn(
        ...
        h_sa_xe2, h_xb_e2_s = (
            min_state_action_to_state_metric(sa_rep, x_e2_rep),
            min_state_action_to_state_metric(xb_e2_rep, s_rep),
        )
        score_p1_e2, score_p2_e2 = (
            (h_sa_xe2 - lambda_e2_target) / beta,
            (h_xb_e2_s - lambda_e2_target) / beta,
        )
        ...
        h_sax_2, h_xbs_2 = (
            min_state_action_to_state_metric(sa_e2_rep, x_e2_rep),
            min_state_action_to_state_metric(xb_e2_rep, s_e2_rep),
        )
        score_p1_2, score_p2_2 = (
            (h_sax_2 - lambda_2_target) / beta,
            (h_xbs_2 - lambda_2_target) / beta,
        )
    ...
    def state_metric_loss_fn(state_metric: EnsembleStateMetric):
        ...
        h_sa_xe2, h_xb_e2_s = (
            target_state_action_to_state_metric(sa_rep, x_e2_rep),
            target_state_action_to_state_metric(xb_e2_rep, s_rep),
        )
        h_sa_xe2, h_xb_e2_s = (
            jax.lax.stop_gradient(h_sa_xe2),
            jax.lax.stop_gradient(h_xb_e2_s),
        )
        g_sx_e2, g_xe2_s = state_metric(s_rep, x_e2_rep)

        score_p1_e2, score_p2_e2 = (
            (h_sa_xe2 - g_sx_e2) / beta,
            (h_xb_e2_s - g_xe2_s) / beta,
        )
        max_score_e2 = jax.lax.stop_gradient(
            jnp.maximum(score_p1_e2.max(), score_p2_e2.max())
        )
        ...
        h_sax_2, h_xbs_2 = (
            target_state_action_to_state_metric(sa_e2_rep, x_e2_rep),
            target_state_action_to_state_metric(xb_e2_rep, sa_e2_rep),
        )
        h_sax_2, h_xbs_2 = (
            jax.lax.stop_gradient(h_sax_2),
            jax.lax.stop_gradient(h_xbs_2),
        )
        g_sx_2, g_xs_2 = state_metric(s_e2_rep, x_e2_rep)
    ...
    def rep_loss_fn(rep_net: RepNetUnified):
        ...
        loss_e2 = jnp.mean(optax.huber_loss(inter_state_diff, u_target_e2, delta=1.0))

        g_sx_2, g_xs_2 = target_state_metric(s_e2_rep, x_e2_rep)
        u_target_2 = jax.lax.stop_gradient(jnp.maximum(g_sx_2, g_xs_2))
        s2_diff = jnp.linalg.norm(s_e2_rep - x_e2_rep, axis=-1, ord=1)
        loss_2 = jnp.mean(optax.huber_loss(s2_diff, u_target_2, delta=1.0))
...
def transfer_tuning(
    ...
    (s2, (rep_net, state_metric), s_eq) = nnx.fori_loop(
        ...
        grad_steps,
        state_grad_steps,
        (s2, (rep_net, state_metric), jnp.zeros_like(s)),
    )
    ...
    (s_eq, pi, s2, (rep_net, state_action_metric), pi_eq) = nnx.fori_loop(
        ...
        grad_steps,
        action_grad_steps,
        (s_eq, pi, s2, (rep_net, state_action_metric), jnp.zeros_like(a2)),
    )
...
def transfer_train_n_step(
    ...
    def body_fun(i, carry):
        ...
        init_val = (jnp.zeros((), jnp.float32),) * 3
        models = (
            rep_net,
            rep_net_opt,
            state_metric,
...
def main(args, cfg_env=None):
    ...
    env2_key = jax.random.split(env2_key, 1)
    ...
    prng_key, buffer_key = jax.random.split(prng_key)
    env_state, buffer_state = prefill_buffer_source(
        key=buffer_key,
        env=env,
        env_state=env_state,
    ...
    prng_key, buffer_e2_key = jax.random.split(prng_key)
    env2_state, buffer_state_e2 = prefill_buffer_target(
        key=buffer_e2_key,
        env=env2,
    ...
    while steps < config["total_env_steps"]:
        ...
        logger.log_tabular("Loss/Loss_representation", rep_loss.item())
        logger.log_tabular("Loss/Loss_actor_transfer", transfer_loss.item())
        logger.log_tabular("Loss/State_matching_loss", state_matching_loss.item())
        logger.log_tabular("Loss/Action_matching_loss", action_matching_loss.item())
