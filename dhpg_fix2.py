...
=========================================================================
SYNCED FROM THE CURRENT sac_single.py  (grep "[SYNC")
=========================================================================
[S1] TRUNCATION MASKING, APPLIED TO EVERY CONTAMINATED TERM.
    SAC drops truncated transitions from the critic loss via
    `q_error *= (1 - truncation)`. DHPG needs the same mask in FIVE places,
    because BraxAutoResetWrapper overwrites state.obs on done: at a
    truncated step `next_observation` is the first obs of the NEW episode,
    and the window`s `reward` is only a partial k<n sum. Contaminated:
        critic TD          <- next_obs via target_Q          masked
        abstract critic TD <- next_obs via next_z            masked
        transition NLL     <- f(next_obs) as regression tgt  masked
        reward consistency <- partial-sum reward             masked
        lax bisimulation   <- reward, reward[perm], g^k      masked (PAIRWISE)
    The actor loss consumes only `obs` and needs no mask.
    The lax term is masked with `valid * valid[perm]`, not `valid`: the
    random permutation couples sample i with perm[i], so a contaminated
    PARTNER poisons an otherwise clean sample`s target. One-sided masking
    would leave those through.
    This required utils/nstep.py to emit a WINDOW-level truncation flag -
    1.0 iff the n-step window ended at a time limit - rather than copying
    the flag of the step the window started from. For n=1 the new flag is
    identical to the old, so 1-step consumers are unaffected.
    At n=3 one truncation contaminates THREE consecutive windows, so this
    matters 3x more than at n=1 (~0.3% of a batch on 1000-step episodes).
    Like SAC, the masked terms use a plain masked mean rather than
    renormalising by the valid count - a ~0.3% scale shift, negligible.
    NOTE: no 0.5 factor on the DHPG critic loss. SAC uses
    0.5*mean(square(...)); the author uses plain F.mse_loss, and here the
    critic loss is SUMMED with the homomorphism losses in one backward, so
    a 0.5 would silently reweight critic vs lax/transition/reward.
[S2] REWARD SCALING. `reward * config.reward_scaling`, applied once at the
    top of dhpg_train_step so it also scales the reward-consistency target
    and the r_dist term of the lax bisimulation - internally consistent,
    and the homomorphism is learned in the units the critics see.
[S3] evaluate() COPIED VERBATIM, so both baselines are scored by identical
    code. DeterministicActor gains mean_action() and sample() purely to
    satisfy its interface.
[S4] warmup_samples is now a TRANSITION count (// num_envs).
[S5] Monotone `next_save` checkpointing; `steps = int(buffer.size(...))`.
[S6] New config keys num_eval_envs and reward_scaling, both from sac_args.
[S7] use_author_algo_hparams DEFAULT FLIPPED to False - see [BOUNDARY].

NOT synced, because they are SAC-specific with no DHPG analogue: the
-0.5*|A| target entropy, the brax-form alpha loss, and SAC's new
compute-all-grads-then-step-all ordering. DHPG keeps the author's
SEQUENTIAL ordering (critic -> abstract critic sees updated encoders ->
actor sees updated critics), which is load-bearing for the homomorphism.

ALSO NOT synced: your SAC is currently 1-STEP. DHPG stays at nstep=3 per
the author, so the two baselines differ in return length. That is a
legitimate algorithm difference - the author uses n-step for DHPG - but
state it in the paper, or set nstep=1 here to equalise.
...
- `Eval/Return` is the DETERMINISTIC eval, produced by evaluate() copied
  verbatim from sac_single.py: noise-free policy, num_eval_envs parallel
  episodes, alive-masked return. Both baselines are now scored by identical
  code, which also matches train.py::Workspace.eval() (eval_mode=True).
  running_state is still maintained but unlogged, mirroring SAC's
  commented-out block.
- `warmup_samples` is a TRANSITION count: warmup_iters = warmup_samples //
  num_envs, as in the new sac_single.py. `log_freq` remains an ITERATION
  count, so a log line still covers log_freq * num_envs transitions - also
  as in sac_single.py.
- Checkpointing uses SAC's monotone `next_save` threshold.
- Neither script clips gradients any more: SAC's optax.chain is commented
  out and the author never clipped, so this now agrees by construction.
...
# [FIX B] + [FIX C] n-step returns now live in utils/nstep.py, shared with
# sac_single.py so the two baselines cannot drift on return computation.
...
default_cfg = {
    ...
    "explore_stddev_decay_steps": int(1e5),  # [FIX A]
    ...
    # [FIX C] Now INERT for DHPG: every term that would consume the discount
    # of a truncated window is masked out by `valid`, so this flag no longer
    # changes any gradient. Kept because utils/nstep.py is shared and a
    # 1-step consumer that does NOT mask still needs it.
    "bootstrap_on_truncation": True,
    "matching_dims": True,
    "feature_dim": 50,  # only used when matching_dims=False
    # ---- regime switches ----
    "use_author_algo_hparams": False,  # [BOUNDARY] see the note in main()
    "match_author_utd": False,  # keep False for a matched comparison
    "normalize_obs": True,  # True = SAC parity; author uses raw obs
    "num_eval_envs": 128,  # [SYNC] parallel envs for the deterministic eval
    "reward_scaling": 1.0,  # [SYNC] sac_single.py scales the TD-target reward
}
...
class DeterministicActor(nnx.Module):
    ...
    # [SYNC] sac_single.py's evaluate() calls actor.mean_action / actor.sample.
    # Providing both lets DHPG reuse that function VERBATIM. For a deterministic
    # policy the two coincide - exploration noise is added by ExploratoryActor
    # during rollouts, never during evaluation.
...
def dhpg_train_step(
    ...
    act = data.action
    # [SYNC] sac_single.py: reward * config.reward_scaling in the TD target.
    # Applied once here, so it also scales the reward-consistency target and
    # the r_dist term of the lax bisimulation - internally consistent, and the
    # homomorphism is learned in the same reward units the critics see.
    reward = data.reward * config.reward_scaling
    # [SYNC] sac_single.py masks truncated transitions out of the critic loss.
    # nstep_aggregate emits a WINDOW-level flag: 1.0 iff this n-step window
    # ended at a time limit.
    #
    # Why masking is REQUIRED, not cosmetic: BraxAutoResetWrapper overwrites
    # state.obs on done, so at a truncated step `next_observation` is the
    # first observation of the NEW episode, not the terminal observation of
    # the old one. Any term that consumes next_obs is reading across an
    # episode boundary. The window's `reward` is also only a partial k<n sum
    # (k = steps before the cut), so it is not on the same scale as a clean
    # n-step return.
    #
    # `valid` therefore gates EVERY contaminated term, not just the two TD
    # losses - at n=3 one truncation contaminates three consecutive windows,
    # so this is 3x more frequent than at n=1. The actor loss consumes only
    # `obs` and needs no mask.
    truncation = data.extras["state_extras"]["truncation"]
    valid = 1.0 - truncation  # (B,), 1.0 = usable
    ...
    def joint_loss_fn(
        ...
        # [SYNC] truncation mask, as in sac_single.py's critic loss. NOTE: no
        # 0.5 factor - the author uses plain F.mse_loss, and here the critic
        # loss is SUMMED with the homomorphism losses in one backward, so a
        # 0.5 would silently reweight critic vs lax/transition/reward.
        q_error = (current_Q - target_Q) * valid
        ...
        # [SYNC] the permutation couples sample i with sample perm[i], so a
        # contaminated PARTNER poisons an otherwise clean sample's target.
        # Gate on both ends of the pair.
        pair_valid = valid * valid[perm]
        lax_bisim_loss = jnp.mean(pair_valid * (z_dist - lax_bisimilarity) ** 2)
        ...
        # [SYNC] next_s_bar_target comes from next_obs, which is post-reset
        # for a truncated window - never regress the transition model onto it
        transition_loss = jnp.mean(
            valid[..., None] * (0.5 * diff**2 + jnp.log(sigma_pred))
        )
        ...
        # [SYNC] a truncated window's reward is a partial k<n sum, so it is
        # not the quantity the predictor is meant to fit
        reward_loss = jnp.mean(valid * (pred_next_reward - reward) ** 2)
    ...
    def abstract_critic_loss_fn(abstract_critic):
        ...
        # [SYNC] same truncation mask on the abstract TD loss
        q_bar_error = (current_Q_bar - target_Q_bar) * valid
        return jnp.mean(q_bar_error**2)
...
def train_n_steps(
    ...
    def body_fun(i, carry):
        ...
        def do_train(j, carry):
            key, buffer_state, obs_normalizer, grad_step, models, prev_val = carry
            (
                actor,
                actor_opt,
                target_actor,
            ...
            # [LOGFIX] actor_loss is 0.0 on skipped steps (the nnx.cond skip
            # branch). Only the LAST inner iteration's value is returned, and
            # with train_per_step a multiple of update_every_steps that last
            # step is ALWAYS a skip - so Loss/Loss_actor logged as exactly 0
            # forever, even though the actor was updating fine. Carry the most
            # recent REAL value through skipped steps instead.
            is_actor_update = (grad_step % config.update_every_steps) == 0
            val = val[:5] + (jnp.where(is_actor_update, val[5], prev_val[5]),) + val[6:]
...
# --------------------------------------------------------------------------- #
# [SYNC] evaluate() copied VERBATIM from sac_single.py, so both baselines are
# scored by identical code: deterministic policy, num_eval_envs parallel
# episodes, alive-masked return so post-termination steps are not counted,
# reported as (mean, std) across envs. This also matches the author's
# train.py::Workspace.eval(), which evaluates with eval_mode=True.
# DeterministicActor.mean_action / .sample exist purely to satisfy this
...
def main(args, cfg_env=None):
    ...
    config.update(
        {
            ...
            "num_eval_envs": args.num_eval_envs,  # [SYNC]
            "reward_scaling": args.reward_scaling,  # [SYNC]
        }
    )

    # ── [BOUNDARY] the only place harness and algorithm touch ─────────────
    # The author uses lr=1e-4 and tau=0.01 where sac_args defaults to 1e-3
    # and 0.005. DEFAULT IS NOW False - i.e. DHPG takes SAC's values.
    #
    # Why the flip: the author's 1e-4 is calibrated to UTD=1 and ~1e6
    # gradient steps. This harness runs UTD = train_per_step/num_envs, so a
    # 5e6-transition run does only total/num_envs*train_per_step gradient
    # steps - far fewer. Transplanting lr alone, without UTD, leaves DHPG
    # badly under-trained, which is the most likely cause of the FingerSpin
    # regression you observed. lr and UTD are coupled; they do not move
    # independently.
    #
    # Set True to recover the author's literal values. Worth running both -
    # the resolved value is printed in the [algorithm] log line.
    ...
    # [SYNC] sac_single.py: warmup_iters = max(1, warmup_samples // num_envs),
    # i.e. warmup_samples is now a TRANSITION count, not an iteration count.
    ...
    nstep_fifo = nstep_fifo_init(
        nstep_template_from_dims(num_envs, obs_dim, act_dim), config["nstep"]
    )
    ...
    # ── [BASELINE] score the UNTRAINED policy before any gradient step ────
    # The first log line lands log_freq*num_envs transitions in, so without
    # this you never observe the true starting point and cannot tell a real
    # learning curve from an env whose random-policy return is simply high.
    ...
    steps = int(buffer.size(buffer_state))  # [SYNC]
    grad_step = jnp.array(0, dtype=jnp.int32)
    next_save = steps + config["save_freq"]  # [SYNC] monotone, cannot miss
    ...
    while steps < config["total_env_steps"]:
        ...
        # [SYNC] Eval/Return is now the DETERMINISTIC eval, exactly as in
        # sac_single.py. The old running_state statistic is left computed but
        # unlogged, mirroring SAC's commented-out block.
        ...
        # ── [SYNC] periodic checkpoint, sac_single.py's monotone form ─────
        if steps >= next_save:
