...
def sac_train_step(
    ...
    ## source-source action matching
    pi, _ = actor(s, act_key)

    def compute_state_diff(
        s: jnp.ndarray, x: jnp.ndarray, state_metric: EnsembleStateMetric
    ):
        g_sx, g_xs = state_metric.get_source_distance(s, x)
    ...
    def compute_state_action_diff(
        ...
        d_spi_xb, d_xb_spi = state_action_metric.get_source_distance(
    ...
    def state_grad_steps(i, carry):
        ...
        grad_s = jax.grad(lambda s: compute_state_diff(x, s, state_metric))(s_eq)
        updates, opt_state = opt.update(grad_s, opt_state)
        s_eq = optax.apply_updates(s_eq, updates)
        return (x, state_metric, s_eq)

    # get the equivalent state w.r.t state_metric.get_source_distance for s
    ...
    # get the equivalent actions for (s, pi) with state_action_metric.get_source_distance
    ...
    def act_match_loss_fn(actor, s_eq, pi_eq):
    ...
    matching_loss, actor_matching_grads = nnx.value_and_grad(act_match_loss_fn)(
        actor, s_eq, pi_eq
    )

    actor_grads_total = jax.tree_util.tree_map(
        lambda a, b: a + b, actor_grads, actor_matching_grads
    )
    actor_opt.update(actor, actor_grads_total)

    ## cross actor matching

    def compute_cross_state_diff(
        s: jnp.ndarray, x: jnp.ndarray, state_metric: EnsembleStateMetric
    ):
        g_sx, g_xs = state_metric.get_cross_distance(s, x)
        u = jnp.maximum(g_sx, g_xs)
        return jnp.mean(u)

    def compute_cross_state_action_diff(
        s: jnp.ndarray,
        pi: jnp.ndarray,
        x: jnp.ndarray,
        b: jnp.ndarray,
        ...
        d_spi_xb, d_xb_spi = state_action_metric.get_cross_distance(
            jnp.concatenate([s, pi], axis=-1), jnp.concatenate([x, b], axis=-1)
        )
        return jnp.mean(jnp.maximum(d_spi_xb, d_xb_spi))

    def cross_state_grad_steps(i, carry):
        (x, state_metric, s_eq) = carry
        opt = optax.adam(config.lr)
        opt_state = opt.init(s_eq)
        grad_s = jax.grad(lambda s: compute_cross_state_diff(x, s, state_metric))(s_eq)
        updates, opt_state = opt.update(grad_s, opt_state)
        s_eq = optax.apply_updates(s_eq, updates)
        return (x, state_metric, s_eq)

    def cross_action_grad_steps(i, carry):
        (s, pi, x, state_action_metric, b_eq) = carry
        opt = optax.adam(config.lr)
        opt_state = opt.init(b_eq)
        grads_b = jax.grad(
            lambda b: jnp.mean(
                compute_cross_state_action_diff(s, pi, x, b, state_action_metric)
            )
        )(b_eq)
        updates, opt_state = opt.update(grads_b, opt_state)
        b_eq = optax.apply_updates(b_eq, updates)
    ...
    def cross_act_match_loss_fn(actor, s_eq, pi_eq):
        actions, _ = actor(s_eq, act_e2_key)
        return jnp.mean(
            optax.huber_loss(actions, jax.lax.stop_gradient(pi_eq), delta=1.0)
        )

    (s, state_metric, s2_eq) = nnx.fori_loop(
        0,
        grad_steps,
        cross_state_grad_steps,
        (s, state_metric, jnp.zeros_like(s2)),
    )
    # given state and it's action pair, and the equivalent state, find the equivalent action
    (s, pi, s2_eq, state_action_metric, pi2_eq) = nnx.fori_loop(
        0,
        grad_steps,
        cross_action_grad_steps,
        (s, pi, s2_eq, state_action_metric, jnp.zeros_like(a2)),
    )
    cross_matching_loss, cross_actor_matching_grads = nnx.value_and_grad(
        cross_act_match_loss_fn
    )(actor_e2, s2_eq, pi2_eq)

    # target-target matching

    def compute_target_state_diff(
        s: jnp.ndarray, x: jnp.ndarray, state_metric: EnsembleStateMetric
    ):
        g_sx, g_xs = state_metric.get_target_distance(s, x)
        u = jnp.maximum(g_sx, g_xs)
        return jnp.mean(u)

    def compute_target_state_action_diff(
        s: jnp.ndarray,
        pi: jnp.ndarray,
        x: jnp.ndarray,
        b: jnp.ndarray,
        ...
        d_spi_xb, d_xb_spi = state_action_metric.get_target_distance(
            jnp.concatenate([s, pi], axis=-1), jnp.concatenate([x, b], axis=-1)
        )
        return jnp.mean(jnp.maximum(d_spi_xb, d_xb_spi))

    def target_state_grad_steps(i, carry):
        (x, state_metric, s_eq) = carry
        opt = optax.adam(config.lr)
        opt_state = opt.init(s_eq)
        grad_s = jax.grad(lambda s: compute_target_state_diff(x, s, state_metric))(s_eq)
        updates, opt_state = opt.update(grad_s, opt_state)
        s_eq = optax.apply_updates(s_eq, updates)
        return (x, state_metric, s_eq)

    def target_action_grad_steps(i, carry):
        (s, pi, x, state_action_metric, b_eq) = carry
        opt = optax.adam(config.lr)
        opt_state = opt.init(b_eq)
        grads_b = jax.grad(
            lambda b: jnp.mean(
                compute_target_state_action_diff(s, pi, x, b, state_action_metric)
            )
        )(b_eq)
        updates, opt_state = opt.update(grads_b, opt_state)
        b_eq = optax.apply_updates(b_eq, updates)
    ...
    def target_act_match_loss_fn(actor, s_eq, pi_eq):
        actions, _ = actor(s_eq, act_e2_key)
        return jnp.mean(
            optax.huber_loss(actions, jax.lax.stop_gradient(pi_eq), delta=1.0)
        )

    pi2, log_pi2 = actor_e2(s2, act_e2_key)
    (s2, state_metric, x2_eq) = nnx.fori_loop(
        0,
        grad_steps,
        target_state_grad_steps,
        (s2, state_metric, jnp.zeros_like(s2)),
    )
    # given state and it's action pair, and the equivalent state, find the equivalent action
    (s2, pi2, x2_eq, state_action_metric, xpi2_eq) = nnx.fori_loop(
        0,
        grad_steps,
        target_action_grad_steps,
        (s2, pi2, x2_eq, state_action_metric, jnp.zeros_like(a2)),
    )
    target_matching_loss, target_actor_matching_grads = nnx.value_and_grad(
        target_act_match_loss_fn
    )(actor_e2, x2_eq, xpi2_eq)

    actor_e2_grads_total = jax.tree_util.tree_map(
        lambda a, b: a + b, cross_actor_matching_grads, target_actor_matching_grads
    )
    actor_e2_opt.update(actor_e2, actor_e2_grads_total)
...
def train_n_steps(
    ...
    def body_fun(i, carry):
        ...
        init_val = (jnp.zeros((), jnp.float32),) * 13
        models = (
            state_metric,
            state_metric_opt,
            state_action_metric,
...
def transfer_step(
    ...
    def find_equivalent_states(i, carry):
        ...
        s_s_eq, s_t_eq, t_s_eq, t_t_eq = states_eq
        ...
        grad_st = jax.grad(
            lambda target: compute_cross_diff(source_state, target, metric)
        )(s_t_eq)
        ...
        grad_ts = jax.grad(
            lambda source_eq: compute_cross_diff(source_eq, target_state, metric)
        )(t_s_eq)
        ...
        grad_tt = jax.grad(
            lambda target_eq: compute_target_diff(target_state, target_eq, metric)
        )(t_t_eq)
        tt_updates, tt_opt_state = tt_opt.update(grad_tt, tt_opt_state)
        t_t_eq = optax.apply_updates(t_t_eq, tt_updates)

        states_eq = (s_s_eq, s_t_eq, t_s_eq, t_t_eq)
        return (states, metric, states_eq)

    ((s, s2), state_metric, (s_s_eq, s_t_eq, t_s_eq, t_t_eq)) = nnx.fori_loop(
        0,
        grad_steps,
    ...
    def find_equivalent_actions(i, carry):
        ...
        ss_eq, st_eq, ts_eq, tt_eq = states_eq
        ss_eq_a, st_eq_b, ts_eq_a, tt_eq_b = action_eq
        ...
        grad_ss_eq_a = jax.grad(
            lambda act_eq: source_state_action_diff(
        ...
        grad_st_eq_b = jax.grad(
            lambda act_eq: cross_state_action_diff(
        ...
        grad_tt_eq_b = jax.grad(
            lambda act_eq: target_state_action_diff(
        ...
        action_eq = (ss_eq_a, st_eq_b, ts_eq_a, tt_eq_b)
        return (states, actions, states_eq, metric, action_eq)

    (
        (s, s2),
        (a, a2),
        (s_s_eq, s_t_eq, t_s_eq, t_t_eq),
        state_action_metric,
        (ss_eq_a, st_eq_b, ts_eq_a, tt_eq_b),
    ) = nnx.fori_loop(
        0,
        grad_steps,
...
def main(args, cfg_env=None):
    ...
    while steps < config["total_env_steps"]:
        ...
        logger.log_tabular("Loss/source_action_matching_loss", matching_loss.item())
        logger.log_tabular(
            "Loss/cross_action_matching_loss", cross_matching_loss.item()
        )
        logger.log_tabular(
            "Loss/target_action_matching_loss", target_matching_loss.item()
        )
