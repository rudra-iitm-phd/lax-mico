...
default_cfg = {
    ...
    "rep_lr_scale": 1.5,
    "rep_anneal_frac": 0.4,  # weight hits the floor at 40% of total_env_steps
    "rep_floor_frac": 0.1,  # residual weight = 0.1 * rep_lr_scale
    "near_pair_scale": 1.0,
}
...
def sac_train_step(
    state: TrainingState, data: Transition, config, step, key: jnp.ndarray
    ...
    key, key_alpha, key_critic, key_actor, key_actor_rep = jax.random.split(key, 5)
    ...
    key, perm_key, env_key_sel = jax.random.split(key, 3)
    batch = jax.random.permutation(perm_key, batch)
    env_idx = jax.random.randint(env_key_sel, (batch.shape[0],), 0, batch.shape[1])
    batch = jnp.take_along_axis(batch, env_idx[:, None, None], axis=1)[:, 0]
    ...
    def actor_loss_fn(actor):
        ...
        d1, d2 = state.models.target_state_action_metric(
            jnp.concatenate([s, pi], axis=-1),
            jnp.concatenate([x, pi_x], axis=-1),
        )
        d = jnp.maximum(d1, d2)
        d_scale = jax.lax.stop_gradient(jnp.mean(d) + 1e-6)
        u_n = u / (jax.lax.stop_gradient(jnp.mean(u)) + 1e-6)
        d_ref = jax.lax.stop_gradient(jnp.mean(d))
        rep_loss = jnp.mean((1.0 - u) * d + u * jax.nn.relu(d_ref - d))
        # rep_loss = jnp.mean(jax.nn.relu(1.0 - u_n) * (d / d_scale))
        # rep_loss = jnp.mean((1.0 - u) * d + u * jax.nn.relu(1.0 - d))

        frac = jnp.clip(
            step / (config.rep_anneal_frac * config.total_env_steps), 0.0, 1.0
        )
        rep_w = config.rep_lr_scale * (1.0 - (1.0 - config.rep_floor_frac) * frac)
        loss = sac_loss + rep_w * rep_loss
        return loss, (jnp.mean(log_pi), sac_loss, rep_loss, rep_w)

    (actor_loss_tot, (log_pi_mean, actor_loss, act_rep_loss, rep_w)), actor_grads = (
    ...
    def state_action_metric_loss_fn(state_action_metric: EnsembleStateActionMetric):
        r_fwd = jax.nn.relu(r - y)  # s side has more reward -> costly to go down
        r_bwd = jax.nn.relu(y - r)
        lambda_target_1 = jax.lax.stop_gradient(r_fwd + discount * g_sx_next)
        lambda_target_2 = jax.lax.stop_gradient(r_bwd + discount * g_xs_next)
    ...
    def state_metric_loss_fn(state_metric: EnsembleStateMetric):
        ...
        # g_ssn1, g_ssn2 = state_metric(s, s_next)
        # near_target = jax.lax.stop_gradient(
        #     jnp.abs(reward[:, -1][:, None]) * config.near_pair_scale
        # )
        # near_loss = jnp.mean((jnp.maximum(g_ssn1, g_ssn2) - near_target) ** 2)
        loss = (
            jnp.mean(p1)
            + jnp.mean(p2)
            + 0.1 * jnp.mean((1 - g_sx) ** 2)
            + 0.1 * jnp.mean((1 - g_xs) ** 2)
...
def train_n_steps(
    ...
    def body_fun(i, carry):
        ...
        step_now = step_base + i * config.num_envs
...
def main(args, cfg_env=None):
    ...
    while steps < config["total_env_steps"]:
        ...
        val = train_n_steps(
            ...
            nstep_fifo=nstep_fifo,  # NEW arg
            nstep_count=nstep_count,  # NEW arg
            step_base=jnp.asarray(steps, dtype=jnp.float32),
            key=subkey,
        )
        ...
        logger.log_tabular("Loss/rep_weight", metric_aux.rep_w.item())
