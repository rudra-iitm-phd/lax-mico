...
PALETTE = ["#e69f00", "#7a8f00", "#b57bb3", "#d97662",
           "#0072b2", "#009e73", "#56b4e9", "#cc79a7"]
...
def load_run(run_dir, args):
    ...
    if args.smooth > 1:
        smooth = pd.Series(vals).rolling(
            args.smooth, center=True, min_periods=1).mean().to_numpy()
    else:
        smooth = vals
...
def outside_legend(fig, algos, labels, colors, markers, ncol=None,
                   fontsize=None, marker_size=6.5, line_width=2.0):
    """Framed legend floating above the axes, as in the reference figures."""
    handles = [Line2D([], [], color=colors[a], marker=markers[a],
                      lw=line_width, ms=marker_size, label=labels[a])
               for a in algos]
...
def fig_aggregate_metrics(scores, cis, algos, labels, colors, markers,
                          args, tag):
    """Point estimate + 95% CI per aggregate metric, one panel each."""
    n = len(algos)
    height = max(0.42 * n + 1.05, 1.6)
    fig, axes = plt.subplots(
        1, len(AGG_NAMES), figsize=(args.fig_width, height),
    ...
    for k, (ax, name) in enumerate(zip(axes, AGG_NAMES)):
        ...
        for y, algo in zip(ypos, algos):
            ...
            ax.errorbar(pt, y, xerr=[[pt - lo], [hi - pt]],
                        color=colors[algo], marker=markers[algo], ms=7,
                        lw=2.0, capsize=3.5, capthick=2.0, zorder=3)
            lo_all.append(lo)
            hi_all.append(hi)
...
def fig_performance_profile(prof, prof_cis, taus, algos, labels,
                            colors, markers, args, tag):
    fig, ax = plt.subplots(figsize=(args.fig_width * 0.52, args.fig_width * 0.42),
                           constrained_layout=True)
    for i, algo in enumerate(algos):
        ax.plot(taus, prof[algo], color=colors[algo], marker=markers[algo],
                markevery=marker_every(len(taus), i, len(algos)), zorder=3)
        ax.fill_between(taus, prof_cis[algo][0], prof_cis[algo][1],
                        color=colors[algo], alpha=0.3, lw=0, zorder=2)
...
def fig_sample_efficiency(frames, iqm, iqm_cis, algos, labels,
                          colors, markers, args, tag):
    fig, ax = plt.subplots(figsize=(args.fig_width * 0.52, args.fig_width * 0.42),
                           constrained_layout=True)
    div, suffix = step_scale(frames[-1])
    x = frames / div
    for i, algo in enumerate(algos):
        ax.plot(x, iqm[algo], color=colors[algo], marker=markers[algo],
                markevery=marker_every(len(x), i, len(algos)), zorder=3)
        ax.fill_between(x, iqm_cis[algo][0], iqm_cis[algo][1],
                        color=colors[algo], alpha=0.3, lw=0, zorder=2)
...
def fig_probability_of_improvement(poi_rows, labels, colors, markers, args):
    ...
    for y, row in zip(ypos, poi_rows):
        color = colors[row["x"]]
        p = row["p_improvement"]
        ax.errorbar(p, y, xerr=[[p - row["ci_low"]], [row["ci_high"] - p]],
                    color=color, marker=markers[row["x"]], ms=7, lw=2.0,
                    capsize=3.5, capthick=2.0, zorder=3)
    ax.axvline(0.5, color="#555555", lw=1.2, ls="--", zorder=2)
    ax.set_yticks(ypos)
    ax.set_yticklabels([f"{labels[r['x']]} vs. {labels[r['y']]}"
                        for r in poi_rows])
...
def fig_grid(curves, frames, tasks, algos, labels, colors, markers, *,
             tag="Return",
             # --- which panels, and in what order --------------------------
             envs=None, titles=None, ncols=4,
             # --- geometry, all in inches ----------------------------------
             panel_width=1.35, panel_height=1.45,
             extra_width=0.75, extra_height=0.95,
             w_pad=0.04, h_pad=0.04,
             margin_left=0.0, margin_right=0.0,
             # --- what the lines look like ---------------------------------
             line_width=1.8, marker_size=5.5, markers_per_curve=5,
             show_markers=True, band="sem", band_alpha=0.3,
             # --- text sizes; None inherits the global style ---------------
             title_size=None, tick_size=None, label_size=None, legend_size=None,
             # --- axes -----------------------------------------------------
             xlabel=None, ylabel=None, xlim=None, ylim=None,
             xticks=3, yticks=None, sharey=False, logx=False,
             # --- legend ---------------------------------------------------
             legend="above", legend_ncol=None, legend_loc="lower right", facecolor=None):
    ...
    fig, axes = plt.subplots(
        nrows, ncols, sharex=True, sharey=sharey, squeeze=False,
        constrained_layout=True,
        figsize=(ncols * panel_width + extra_width,
    ...
    for idx, env in enumerate(envs):
        ...
        for i, algo in enumerate(algos):
            ...
            ax.plot(x, mean, color=colors[algo], lw=line_width,
            ...
            if band and runs.shape[0] > 1:
                ...
                ax.fill_between(x, lo, hi, color=colors[algo],
    ...
    fig.supxlabel(xlabel if xlabel is not None
                  else f"environment steps {suffix}".strip(),
                  fontsize=label_size)
    fig.supylabel(ylabel if ylabel is not None else f"Normalized {tag}",
                  fontsize=label_size)

    if legend == "above":
        outside_legend(fig, algos, labels, colors, markers, ncol=legend_ncol,
                       fontsize=legend_size, marker_size=marker_size,
                       line_width=line_width)
    elif legend == "inside":
        handles = [Line2D([], [], color=colors[a], lw=line_width,
                          marker=markers[a] if show_markers else None,
                          ms=marker_size, label=labels[a]) for a in algos]
...
def fig_per_env_curves(curves, frames, tasks, algos, labels,
                       colors, markers, args, tag):
    ...
    return fig_grid(
        ...
        ncols=ncols, panel_width=panel_w, panel_height=args.panel_height,
        w_pad=pad, h_pad=pad,
...
def run_analysis(final, curves, frames, tasks, args):
    ...
    print(f"\n=== Aggregate metrics (stratified bootstrap, {args.reps} reps, "
          f"95% CI) ===")
    ...
    for algo in algos:
        for k, name in enumerate(AGG_NAMES):
            rows.append(dict(algorithm=algo, label=labels[algo], metric=name,
    ...
    for algo in algos:
        ...
        for _, r in agg_df[agg_df.algorithm == algo].iterrows():
            print(f"  {r.metric:<15} {r.point:7.4f}  "
                  f"[{r.ci_low:7.4f}, {r.ci_high:7.4f}]")
    agg_df.to_csv(os.path.join(args.out_dir, f"aggregate_metrics_{tag}.csv"),
                  index=False)
    written += save(
        fig_aggregate_metrics(agg_scores, agg_cis, algos, labels, colors,
                              markers, args, tag),
    ...
    written += save(
        fig_performance_profile(prof, prof_cis, taus, algos, labels,
                                colors, markers, args, tag),
    ...
    written += save(
        fig_sample_efficiency(frames, iqm_scores, iqm_cis, algos, labels,
                              colors, markers, args, tag),
        args.out_dir, f"sample_efficiency_{tag}", args.formats)

    pd.DataFrame([
        dict(algorithm=algo, step=fr, iqm=iqm_scores[algo][t],
             ci_low=iqm_cis[algo][0, t], ci_high=iqm_cis[algo][1, t])
        for algo in algos for t, fr in enumerate(frames)
    ]).to_csv(os.path.join(args.out_dir, f"sample_efficiency_{tag}.csv"),
              index=False)
    ...
    if len(algos) >= 2:
        ...
        for key in pairs:
            ...
            print(f"  P({labels[x]} > {labels[y]}) = {p:.3f} "
                  f"[{ci[0]:.3f}, {ci[1]:.3f}]")
            poi_rows.append(dict(x=x, y=y, p_improvement=p,
    ...
    grid = fig_per_env_curves(curves, frames, tasks, algos, labels,
                              colors, markers, args, tag)
    written += save(grid, args.out_dir, f"per_env_curves_{tag}", args.formats,
                    tight=not (args.margin_left or args.margin_right))

    # ---- 6. per-run table + LaTeX -----------------------------------------
    raw_df = pd.DataFrame([
        dict(algorithm=algo, env=env, run_index=i,
    ...
    raw_df.to_csv(os.path.join(args.out_dir, f"per_run_final_{tag}.csv"),
                  index=False)
    print(f"\n=== Per-run final {tag} (mean of last {args.final_window} evals) ===")
    print(raw_df.pivot_table(index=["env", "run_index"], columns="algorithm",
                             values="raw_score").round(2).to_string())
    ...
    with open(os.path.join(args.out_dir, f"summary_{tag}.json"), "w") as fh:
        json.dump({"aggregate": agg_df.to_dict("records"),
                   "probability_of_improvement": poi_rows,
                   "tasks": tasks, "algorithms": algos}, fh, indent=2)

    print(f"\nWrote {len(written)} figure/table files to "
          f"{os.path.abspath(args.out_dir)}")
    print("Per-environment panels show the mean over seeds with a ±1 s.e.m. "
...
def build_parser():
    ...
    p.add_argument("--envs", nargs="+", default=["all"],
                   help="environment names, or 'all'")
    ...
    p.add_argument("--max-score", type=float, default=1000.0,
    ...
    p.add_argument("--drop-leading-zeros", action="store_true",
    ...
    g.add_argument("--label", action="append", dest="label_pairs", default=[],
                   metavar="ALGO=NAME",
                   help="camera-ready name, e.g. --label algo3=Ours")
    g.add_argument("--color", action="append", dest="color_pairs", default=[],
                   metavar="ALGO=HEX", help="override a colour, e.g. algo3=#0173B2")
    g.add_argument("--fig-width", type=float, default=5.5,
    ...
    g.add_argument("--grid-cols", type=int, default=4,
                   help="columns in the per-environment grid")
    g.add_argument("--panel-width", type=float, default=None,
                   help="width of one grid panel in inches; default splits "
                        "--fig-width across the columns")
    g.add_argument("--panel-height", type=float, default=1.45,
                   help="height of one grid panel in inches")
    g.add_argument("--panel-pad", type=float, default=None,
                   help="space between grid panels in inches (default ~0.04)")
    g.add_argument("--margin-right", type=float, default=0.0,
                   help="blank margin reserved on the right, in inches; use it "
                        "to balance the y-label space on the left")
    g.add_argument("--margin-left", type=float, default=0.0,
                   help="blank margin reserved on the left, in inches")
    g.add_argument("--formats", nargs="+", default=["pdf"],
...
def main():
    ...
    if n_seeds * len(tasks) < 10:
        print("NOTE: with this few runs x tasks the bootstrap CIs are wide and "
